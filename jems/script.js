...
function lastTopic(){const text=recentUsers(3);if(text.includes('study resource')||text.includes('resource manager'))return'study';if(text.includes('productivity')||text.includes('focus timer')||text.includes('dashboard'))return'productivity';if(text.includes('student record')||text.includes('c project'))return'cproject';if(text.includes('project')||text.includes('banaya')||text.includes('built'))return'projects';return''}
function answer(q){
 ...
 if((s.includes('ye')||s.includes('this')||s.includes('that')||s.includes('it')||s.includes('iske')||s.includes('iska'))&&(s.includes('kya')||s.includes('kaise')||s.includes('explain')||s.includes('detail')||s.includes('more'))){if(topic==='study')return facts.study;if(topic==='productivity')return facts.productivity;if(topic==='cproject')return facts.cproject;if(topic==='projects')return facts.projects}
...
clearBtn.addEventListener('click',()=>{if(!history.length)return;if(!confirm('Clear the Jems conversation from this browser?'))return;localStorage.removeItem(STORAGE_KEY);history.length=0;messages.replaceChildren();add(`All clear. 👋 Fresh conversation from here. Bolo, kya explore karna hai?`,'bot');persist()});
themeBtn.addEventListener('click',()=>{document.body.classList.toggle('light');localStorage.setItem(THEME_KEY,document.body.classList.contains('light')?'light':'dark')});

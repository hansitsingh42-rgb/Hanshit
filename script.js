...
if(localStorage.getItem('hanshit-theme')==='dark')document.body.classList.add('dark');
...
$('#closeModal').addEventListener('click',closeModal);$('#modalAction').addEventListener('click',closeModal);$('#projectModal').addEventListener('click',(e)=>{if(e.target.id==='projectModal')closeModal()});
document.addEventListener('keydown',(e)=>{if(e.key==='Escape'&&!$('#projectModal').hidden)closeModal()});
const search=$('#librarySearch');const cards=$$('.library-card');
function filterLibrary(){const term=search.value.trim().toLowerCase();let visible=0;cards.forEach(card=>{const ok=card.dataset.name.toLowerCase().includes(term);card.hidden=!ok;if(ok)visible++});$('#libraryEmpty').hidden=visible!==0}
search.addEventListener('input',filterLibrary);
cards.forEach(card=>card.addEventListener('click',()=>{showToast(`${card.dataset.name} selected — resources can be added here.`)}));

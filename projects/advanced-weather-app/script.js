...
async function searchPlaces(query,showSuggestions=false){
  ...
  }catch{setStatus('Could not search for that location. Try again.',true)}
}

async function loadWeather(place){
  currentPlace=place;$('suggestions').hidden=true;setStatus('Loading live weather…');
  const params=new URLSearchParams({latitude:place.latitude,longitude:place.longitude,timezone:'auto',forecast_days:'7',current:'temperature_2m,relative_humidity_2m,apparent_temperature,is_day,precipitation,weather_code,cloud_cover,surface_pressure,wind_speed_10m,visibility,uv_index',hourly:'temperature_2m,apparent_temperature,precipitation_probability,weather_code,wind_speed_10m',daily:'weather_code,temperature_2m_max,temperature_2m_min,sunrise,sunset,uv_index_max,precipitation_probability_max,precipitation_sum'});
  try{const res=await fetch(`${API}?${params}`);if(!res.ok)throw new Error('Weather API failed');const data=await res.json();renderWeather(data);setStatus('Live data loaded.')}catch{setStatus('Weather data could not be loaded. Check your connection and try again.',true)}
}
...
function renderHourly(data){const nowIndex=Math.max(0,data.hourly.time.findIndex(t=>t>=data.current.time));$('hourlyForecast').innerHTML=data.hourly.time.slice(nowIndex,nowIndex+12).map((t,i)=>{const idx=nowIndex+i,[icon]=weatherMap[data.hourly.weather_code[idx]]||['🌡️'];return `<div class="hour"><small>${i===0?'Now':formatTime(t,data.timezone)}</small><div class="mini-icon">${icon}</div><strong>${temp(data.hourly.temperature_2m[idx])}</strong><small>${data.hourly.precipitation_probability[idx]??0}% rain</small></div>`}).join('')}
function renderDaily(data){$('dailyForecast').innerHTML=data.daily.time.map((t,i)=>{const [icon]=weatherMap[data.daily.weather_code[i]]||['🌡️'];return `<div class="day"><small>${i===0?'Today':formatDay(t,data.timezone)}</small><div class="mini-icon">${icon}</div><div class="range">${temp(data.daily.temperature_2m_max[i])} / ${temp(data.daily.temperature_2m_min[i])}</div><div class="rain">💧 ${data.daily.precipitation_probability_max[i]??0}%</div></div>`}).join('')}
function renderInsights(data){const d=data.daily,c=data.current,max=Math.max(...d.temperature_2m_max),rain=Math.max(...d.precipitation_probability_max),uv=Math.max(...d.uv_index_max);const tips=[{title:'🌡️ Temperature',text:`The warmest day in the 7-day outlook reaches ${temp(max)}.`},{title:'🌧️ Rain watch',text:rain>=60?`Rain probability peaks at ${rain}%. Consider carrying an umbrella.`:`Rain probability stays relatively low, peaking at ${rain}%.`},{title:'🧴 UV awareness',text:uv>=6?`UV peaks around ${uv}. Plan shade and sun protection outdoors.`:`UV peaks around ${uv}, generally lower than high-risk levels.`},{title:'💨 Wind',text:`Current wind is ${wind(c.wind_speed_10m)}.`},{title:'☁️ Sky',text:`Cloud cover is ${c.cloud_cover}% right now.`},{title:'💧 Humidity',text:`Relative humidity is ${c.relative_humidity_2m}%.`}];$('insights').innerHTML=tips.map(x=>`<div class="insight"><strong>${x.title}</strong><span class="muted">${escapeHtml(x.text)}</span></div>`).join('')}
...
$('locationBtn').addEventListener('click',()=>{if(!navigator.geolocation){setStatus('Geolocation is not supported by this browser.',true);return}setStatus('Requesting your location…');navigator.geolocation.getCurrentPosition(pos=>loadWeather({name:'Your Location',country:'',latitude:pos.coords.latitude,longitude:pos.coords.longitude}),()=>setStatus('Location access was unavailable. Search for a city instead.',true),{enableHighAccuracy:true,timeout:10000})});
